...
function widget_ftwp( $args, $widget_args = 1 ) {
		...
		//-------------------------------------------------- *  The loop START
			...
			$my_ftwp_query = new WP_Query('tag='.$my_tag.'&posts_per_page='.$nposts); 
...
function widget_ftwp_control( $widget_args = 1 ) {
        ...
        if ( !$updated && !empty($_POST['sidebar']) ) {
                ...
                foreach ( (array) $_POST['widget-featured-tag'] as $widget_number => $widget_ftwp_instance ) {
                        ...
                        $ftwp_options[$widget_number] = array( 'customtext' => $customtext, 'tag' => $featuredtag, 'title' => $title, 'nposts' => $nposts, 'showimage' => $showimage, 'showtitle' => $showtitle, 'showauthor' => $showauthor );  						// Even simple widgets should store stuff in array, rather than in scalar
                }
